class ParticipantsController < ApplicationController
  ...
  def create
    @participant = Participant.where("player_id = ?", params[:player_id]).where("tour_id = ?", params[:tour_id]).first
    if !@participant
      @participant = Participant.new(participant_params)
    end
    player = Player.find(params[:player_id])
    @participant.ranking = player.ranking
    @participant.points = player.points
    respond_to do |format|
      if @participant.save
        @participants = Participant.where("tour_id = ?", params[:tour_id])
  ...
  private
    ...
    def participant_params
      params.require(:participant).permit(:ranking, :points, :tour_id, :player_id)
    end
end
